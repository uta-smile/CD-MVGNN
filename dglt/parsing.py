...
def add_train_args(parser: ArgumentParser):
    ...
    parser.add_argument('--dataset_type', type=str,
                        choices=['classification', 'regression', 'multiclass'],default='classification',
    ...
    parser.add_argument('--hidden_size', type=float, default=2,
    ...
    # Arguments for MultiMPNN
    parser.add_argument('--nencoders', type=int, default=1,
                        help='The number of encoder in the model (Validate for MultiMPNN).')
    ...
    # Arguments for 3D atom coordinate metirc and 2D atom pair distance
    parser.add_argument('--coord', type=str,
                        help='Add 3D coordinate to bond features')

    parser.add_argument('--use_norm', action='store_true', default=False,
                        help='Use layer normalization or not')
...
def modify_train_args(args: Namespace):
    ...
    if not ((args.dataset_type == 'classification' and args.metric in ['auc', 'prc-auc', 'accuracy']) or
            ...
            (args.dataset_type == 'multiclass' and args.metric in ['accuracy', 'auc', 'prc-auc'])):
    ...
    if args.input_layer == 'gcn':
        assert args.atom_messages and args.model_type == 'mpnn'
    ...
    if args.coord:
        # with open(args.coord, 'rb') as f:
        #     args.coord =  pickle.load(f)
        mols = Chem.SDMolSupplier(args.coord)
        args.coord = {}
        for mol in mols:
            args.coord[Chem.MolToSmiles(mol)] = mol.GetConformer().GetPositions()
