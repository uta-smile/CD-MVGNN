...
from dglt.data.dataset.utils import get_class_sizes, get_data, split_data, get_task_names
from dglt.models import build_model
from dglt.models.nn_utils import param_count, initialize_weights
from dglt.multi_gpu_wrapper import MultiGpuWrapper as mgw
from dglt.train.metrics import get_metric_func
from dglt.utils import build_optimizer, build_lr_scheduler, get_loss_func, load_checkpoint, \
    makedirs, save_checkpoint
from .evaluate import evaluate, evaluate_predictions
from .predict import predict
from .train import train
from .utils import report_progress, heartbeat
...
def run_training(args: Namespace, time_start, logger: Logger = None) -> List[float]:
    ...
    :param time_start: time when it starts the training process. Format: YEAR_MONTH_DAY_HOUR_MINUTE_SECOND
    :param logger: Logger.
    ...
    idx = mgw.local_rank() if args.enbl_multi_gpu else args.gpu
    ...
    if args.save_smiles_splits and master_worker:
    ...
    writer_path = f"{args.save_dir}/norm_{args.use_norm}/{args.model_type}_{dataset_name}_dist_coff_{args.dist_coff}/{time_start}/"
    ...
    for model_idx in range(args.ensemble_size):
        ...
        if args.enbl_multi_gpu:
            ...
            #mgw.broadcast_optimizer_state(optimizer, root_rank=0)
        ...
        debug(model)
        if master_worker: debug(f'Number of parameters = {param_count(model):,}')
        if args.cuda:
        ...
        for epoch in range(args.epochs):
            ...
            if os.environ.get('CHIEF_IP', ''):
                progress_train = {"type": "train", "step": int(epoch), "loss": train_loss, "special_info":{'t_time':t_time}}
                report_progress(progress_train)
                progress_val = {'type': 'test', 'step': int(epoch), 'accuracy': avg_val_score, 'special_info':{'val_loss': val_loss}}
            ...
            if epoch > args.skip_epoch:
                # Save model checkpoint if improved validation score
                if args.select_by_loss:
                    if val_loss < min_val_loss:
                        min_val_loss, best_epoch = val_loss, epoch
                        if master_worker: save_checkpoint(os.path.join(save_dir, 'model.pt'), model, scaler, features_scaler, args)
                else:
        ...
        json_dump = {'best_epoch': best_epoch, 'num_params': param_count(model)}
        if args.select_by_loss:
            json_dump['min_val_loss'] = min_val_loss
        else:
            json_dump['best_score'] = best_score
        json_dump['model'] = model.__str__()
        json_dump['args'] = args.__dict__

        with open(os.path.join(writer_path, f"result_{time_start}.json"), "w") as f:
    ...
    if master_worker:
        ...
        if args.dataset_type == 'multiclass':
            data = np.column_stack([np.argmax(avg_test_preds, -1), np.array([i[0] for i in test_targets])])
            ind = ['pred', 'targets']
        else:
