...
# Atom feature sizes
from dglt.data.transformer.scaler import AdjScaler
...
def get_bond_fdim(args: Namespace) -> int:
    ...
    :param: Arguments.
    """
    return BOND_FDIM if not args.coord else 3
...
class MolGraph:
    ...
    def __init__(self, smiles: str,  args: Namespace):
        ...
        # TODO: add lazy calculation mechanism.
        if self.args is not None and self.args.input_layer == 'gcn':
            adj_scaler = AdjScaler()
            adj = Chem.rdmolops.GetAdjacencyMatrix(mol)
            self.adj = adj_scaler.normalize(adj + sp.eye(adj.shape[0]))
        else:
            self.adj = None

        if args.coord:
            smi = Chem.MolToSmiles(mol)
            if smi in args.coord:
                distances_3d = args.coord[smi]
            else:
                distances_3d = np.zeros((mol.GetNumAtoms(), 3))
            # distances_3d = np.digitize(distances_3d, THREE_D_DISTANCE_BINS)
        else:
            distances_3d = None
        ...
        for a1 in range(self.n_atoms):
            for a2 in range(a1 + 1, self.n_atoms):
                ...
                if args.coord:
                    distance_3d = distances_3d[a1] - distances_3d[a2]
                    rev_distance_3d = distances_3d[a2] - distances_3d[a1]
                else:
                    distance_3d = None
                    rev_distance_3d = None

                f_bond = self.bond_features(bond, distance_3d=distance_3d)
                rev_f_bond = self.bond_features(bond, distance_3d=rev_distance_3d)

                if args.atom_messages or args.coord:
                    self.f_bonds.append(f_bond)
                    self.f_bonds.append(rev_f_bond)
                else:
...
class BatchMolGraph:
    ...
    def __init__(self, mol_graphs: List[MolGraph], args: Namespace):
        ...
        self.bond_fdim = get_bond_fdim(args) + ((not args.atom_messages) * self.atom_fdim if not args.coord else 0)
        ...
        adjs_batch = None
        if mol_graphs[0].adj is not None:
            adjs_batch = linalg.block_diag([1], *[d.adj for d in mol_graphs])
        ...
        self.adjs_batch = adjs_batch  # TODO: not smart here.
    ...
    def get_components(self) -> Tuple[torch.FloatTensor, torch.FloatTensor,
        ...
        """
        return self.f_atoms, self.f_bonds, self.a2b, self.b2a, self.b2revb, self.a_scope, self.b_scope, self.a2a, self.adjs_batch
