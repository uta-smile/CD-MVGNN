...
from dglt.data.transformer.scaler import StandardScaler, AdjScaler
...
class MoleculeDataset(Dataset):
    ...
    def __init__(self, data: List[MoleculeDatapoint]):
        ...
        """
        self.data = data
        # I'm go mad now. Stuck here for near half of an hour.
        self.args = self.data[0].args if len(self.data) > 0 and hasattr(self.data[0], "args") else None
        self.scaler = None
    ...
    def adjs(self) -> List[np.ndarray]:
        """
        Returns the adjacency matrix associated with each molecule.

        :return: A list of 2D numpy arrays containing the adjacency matrix for each molecule or None if input layer is not gcn
        """
        if len(self.data) == 0 or self.data[0].adj is None:
            return None

        return [d.adj for d in self.data]
    ...
    def randomize_smiles(self, datapoint):
        smiles = datapoint.smiles
        m = Chem.MolFromSmiles(smiles)
        ans = list(range(m.GetNumAtoms()))
        np.random.shuffle(ans)
        nm = Chem.RenumberAtoms(m, ans)
        datapoint.smiles = Chem.MolToSmiles(nm, canonical=False, isomericSmiles=True)
        datapoint.mol = Chem.MolFromSmiles(datapoint.smiles)
    ...
    def __getitem__(self, idx) -> Union[MoleculeDatapoint, List[MoleculeDatapoint]]:
        ...
        if self.args is not None:
            if np.random.random() < self.args.aug_rate:
