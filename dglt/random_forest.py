...
import numpy as np
from dglt.train.prediction.evaluate import evaluate_predictions
from sklearn.ensemble import RandomForestClassifier
from sklearn.ensemble import RandomForestRegressor
from tqdm import trange, tqdm

from dglt.data.dataset.molecular import MoleculeDataset
from dglt.data.dataset.utils import get_data, split_data
...
def run_random_forest(args: Namespace, logger: Logger = None) -> List[float]:
    ...
    debug('Computing {}'.format(args.features_generator))

    for dataset in [train_data, test_data]:
        for datapoint in tqdm(dataset, total=len(dataset)):
            features_array = []
            for generator in args.features_generator:
                if generator in ['morgan', 'morgan_count']:
                    array = get_features_generator(generator)(mol=datapoint.smiles, radius=args.radius,
                                                              num_bits=args.num_bits)
                else:
                    array = get_features_generator(generator)(mol=datapoint.smiles)
                features_array.append(array)
            datapoint.set_features(np.concatenate(features_array, axis=0))
