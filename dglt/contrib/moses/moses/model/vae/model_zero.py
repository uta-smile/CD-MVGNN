...
class VAE(nn.Module):
    ...
    def forward_decoder(self, x, z):
        ...
        if self.self_loop:
            ...
            x = torch.tensor([self.pad], device=self.device).repeat(len(x),max(lengths))
        x_emb = self.x_emb(x)
    ...
    def sample(self, n_batch, max_len=100, z=None, temp=1.0):
        ...
        with torch.no_grad():
            ...
            if self.self_loop:
                ...
                t= torch.tensor(self.bos, device=self.device).repeat(n_batch)
            ...
            for i in range(1, max_len):
                if self.self_loop:
                    ...
                    x_emb = self.x_emb(t).unsqueeze(1)
                x_input = torch.cat([x_emb, z_0], dim=-1)
