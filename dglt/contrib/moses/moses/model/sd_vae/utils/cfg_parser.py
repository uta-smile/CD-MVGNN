...
class Grammar(object):
    ...
    def load(self, filepath):
        ...
        # # info for rule matching
        # self.head_to_rules = head_to_rules = {}
        # self.valid_tokens = set(cfg_grammar._lexical_index.keys())
        # rule_ranges = {}
        # total_num_rules = 0
        # first_head = cfg_grammar.productions()[0].lhs()
        #
        # for p in cfg_grammar.productions():
        #     head = p.lhs()
        #     rule = p.rhs()
        #     if head not in head_to_rules:
        #         head_to_rules[head] = [rule]
        #     else:
        #         head_to_rules[head].append(rule)
        #
        # for head, rules in head_to_rules.items():
        #     rule_ranges[head] = (total_num_rules, total_num_rules + len(rules))
        #     total_num_rules += len(rules)
    ...
    def tokenize(self, sent):
        ...
        while i < n:
            j = i
            while j + 1 <= n and (sent[i:j+1] in self.valid_tokens or sent[i:j+1] == 'R'):
                j += 1
            if i == j:
                return None
