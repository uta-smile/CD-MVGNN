...
from dglt.contrib.moses.moses.model.mpnn.utils import makedirs
from dglt.contrib.moses.moses.data.mpnn.features import get_available_features_generators, load_features
from dglt.contrib.moses.moses.model.gvae.config import get_train_parser
from dglt.contrib.moses.moses.data.mpnn.features.features_generators import MORGAN_NUM_BITS
...
def add_train_args(parser: ArgumentParser):
    ...
    parser.add_argument('--data_path', type=str,default='/data2/weiyangxie/mit/drug_data/clintox.csv',
    ...
    parser.add_argument('--nencoders', type=int, default=1, help='The number of encoder in the model.')
...
def modify_train_args(args: Namespace):
    ...
    if args.features_generator is not None:
        ...
        if 'rdkit_2d' in args.features_generator:
            args.features_dim += 200
        if 'morgan' in args.features_generator:
            args.features_dim += MORGAN_NUM_BITS
        if 'morgan_count' in args.features_generator:
            args.features_dim += MORGAN_NUM_BITS

    if args.features_path is not None:
        for feat_path in args.features_path:
            args.feature_dim += load_features(feat_path).shape[1]

    args.num_lrs = 1

    if args.ffn_hidden_size is None:
        args.ffn_hidden_size = args.hidden_size * 100
...
def parse_train_args(parser: ArgumentParser = None) -> Namespace:
    """
