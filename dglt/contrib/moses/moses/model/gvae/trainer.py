...
class GVAETrainer(MosesTrainer):
    ...
    def fit(self, model, train_data, val_data=None):
        ...
        # get GVAE column
        train_data = np.squeeze(train_data['IDXS'].values).tolist()
        val_data = None if val_data is None else np.squeeze(val_data['IDXS'].values).tolist()
        ...
        ttrn_loader = self.get_dataloader(model, train_data[n_tval_samples:], shuffle=True)
        tval_loader = self.get_dataloader(model, train_data[:n_tval_samples], shuffle=True)

        # build a data loader for validation samples, if needed
        val_loader = None
        if val_data is not None:
            val_loader = self.get_dataloader(model, val_data, shuffle=False)
    ...
    def get_collate_fn(self, model):
        ...
        collate_fn = lambda data: [model.raw2tensor(x, device=device) for x in data]
    ...
    def _train_epoch(self, model, epoch, tqdm_data, kl_coeff, optimizer=None):
        ...
        for i, input_batch in enumerate(tqdm_data):
            ...
            input_batch = tuple(x.to(model.device) for x in input_batch)

            # Forward
            kl_loss, recon_loss, acc_all, acc_vld, acc_pad = model(input_batch)
