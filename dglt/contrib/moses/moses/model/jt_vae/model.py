...
from dglt.contrib.moses.moses.model.jt_vae.nnutils import create_var, flatten_tensor, avg_pool
from dglt.contrib.moses.moses.model.jt_vae.jtnn_enc import JTNNEncoder
from dglt.contrib.moses.moses.model.jt_vae.jtnn_dec import JTNNDecoder
from dglt.contrib.moses.moses.model.jt_vae.mpn import MPN
from dglt.contrib.moses.moses.model.jt_vae.jtmpn import JTMPN

from dglt.contrib.moses.moses.model.jt_vae.chemutils import enum_assemble, set_atommap, copy_edit_mol, attach_mols
import rdkit
...
class JTVAE(nn.Module):
    ...
    def __init__(self, vocab, config):
        super(JTVAE, self).__init__()
        self.vocab = vocab
        self.hidden_size = config.hidden_size
        self.latent_size = config.latent_size = config.latent_size // 2 #Tree and Mol has two vectors
    ...
    def forward(self, x_batch, beta):
        ...
        #print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())+": begin encode")
    ...
    def sample(self, n_batch=1, max_len=None, z=None, temp=None, prob_decode=False):
        """Generating n_batch samples in eval mode (`z` could be not on same device)

        :param n_batch: number of graphs to generate, currently only n_batch=1
        :param max_len: max len of smiles, currently no valid in jtvae
        :param z: (z_tree, z_mol) of floats, latent vector z or None, z_tree=z_mol=(1, d_n)
        ...
        with torch.no_grad():
            if z is None:
                z = self.sample_prior()
            z_tree, z_mol = z
            z_tree = z_tree.to(self.device)
            z_mol = z_mol.to(self.device)
            #z_0 = z.unsqueeze(1)

            smiles.append(self.decode(z_tree, z_mol, prob_decode))
            print(smiles)
            return smiles
