...
from dglt.contrib.moses.moses.data.reader.tox21_datasets import load_tox21
  ...
  for dataset in datasets:
    ...
    if split is not None:
      ...
      tasks, all_dataset, transformers = loading_functions[dataset](
          featurizer=featurizer, split=split, reload=reload, K=kfold)
    else:
      tasks, all_dataset, transformers = loading_functions[dataset](
          featurizer=featurizer, reload=reload, K=kfold)
    if kfold <= 1:
        train_dataset, valid_dataset, test_dataset = all_dataset
        #print("Debug: %s"%(type(train_dataset)))
    #print(all_dataset[0])
    ...
    if isinstance(model, str):
      if mode == 'classification':
        benchmarkfunc = benchmark_classification
      elif mode == 'regression':
        benchmarkfunc = benchmark_regression
      
      if kfold > 1:
        train_score, valid_score, test_score = cr_evaluate(
            benchmarkfunc, 
            all_dataset, 
            tasks, 
            transformers, 
        ...
        train_score, valid_score, test_score = benchmarkfunc(
            train_dataset,
            valid_dataset,
            test_dataset,
            tasks,
    ...
    with open(os.path.join(out_path, 'results.csv'), 'a') as f:
      ...
      for i in train_score[model_name]:
        if kfold > 1:
            split_str = "cr_%d_%s"%(kfold, str(split))
        else:
            split_str = str(split)
        output_line = [
            dataset,
            split_str, mode, model_name, i, 'train',
...
def cr_evaluate(benchmarkfunc, alldataset, tasks, transformers, n_features, metric, model, test, hyper_parameters, seed):
    """
    Perform the cross-validation on k-fold datasets.
    ----------
    benchmarkfunc: the function object
      benchmark function of the classification / regression task.
    alldataset: the k-fold dataset
    ...
    -------
    train_scores : dict
       predicting average results(AUC) on training set
    valid_scores : dict
       predicting average results(AUC) on valid set
    test_scores : dict
       predicting average results(AUC) on test set

    """
    train_score = {}
    valid_score = {}
    test_score = {}
    for ds in alldataset:
        train_dataset = ds[0]
        test_dataset = ds[1]
        tr_score, v_score, t_score = benchmarkfunc(
            train_dataset, 
            test_dataset,
            test_dataset,
            tasks,
        ...
        if len(train_score) == 0:
            for k in tr_score:
                ...
                test_score[k] = {}
                for m in tr_score[k]:
                    train_score[k][m] = tr_score[k][m]
                    valid_score[k][m] = v_score[k][m]
                    test_score[k][m]  = t_score[k][m]
        else:
            for k in train_score:
                for m in train_score[k]:
                    train_score[k][m] += tr_score[k][m]
                    valid_score[k][m] += v_score[k][m]
                    test_score[k][m]  += t_score[k][m]    
    ...
    for k in train_score:
        for m in train_score[k]:
            train_score[k][m] /= len(alldataset)
            valid_score[k][m] /= len(alldataset)
            test_score[k][m] /= len(alldataset)
