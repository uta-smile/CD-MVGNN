...
from dglt.contrib.moses.moses.data.mpnn.data.scaffold import log_scaffold_stats, scaffold_split
from dglt.contrib.moses.moses.data.mpnn.features import load_features
...
def get_data(path: str,
    ...
    with open(path) as f:
        ...
        data = MoleculeDataset(list(Parallel(n_jobs=args.n_workers)(
            delayed(MoleculeDatapoint)(
                line=line,
                args=args,
...
import torch
from dglt.contrib.moses.moses.data.mpnn.features import mol2graph
...
class MolCollator(object):
    ...
    def __call__(self, batch):
        ...
        batch = mol2graph(smiles_batch, self.shared_dict, self.args)
        if any([len(tb) > 0 for tb in target_batch]):
            mask = torch.Tensor([[x is not None for x in tb] for tb in target_batch])
            targets = torch.Tensor([[0 if x is None else x for x in tb] for tb in target_batch])
        else:
            mask = None
            targets = None
        return batch, features_batch, mask, targets
