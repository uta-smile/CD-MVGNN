...
from dglt.contrib.moses.moses.benchmark.run_benchmark import run_benchmark
...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="If kfold >= 1, conduct k fold benchmark."
    )
parser.add_argument(
    "-o",
    default=".",
...
for dataset in datasets:
  for split in splitters:
    for model in models:
      np.random.seed(seed)
      print("%s: %s %s"%(dataset, str(model), split))
      run_benchmark(
          [dataset], str(model), split=split, test=test, kfold=kfold,reload=False, seed=seed)
